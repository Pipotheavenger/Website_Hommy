    ...
    <section class="py-24 2xl:py-32 bg-background text-center">
      ...
      <div class="container">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 2xl:gap-20">
          ${renderBenefitItem('Profesionales Verificados', '🛡️', 'Validamos identidad y antecedentes de cada experto para tu tranquilidad.')}
          ${renderBenefitItem('Compara Presupuestos', '⚖️', 'Recibe múltiples ofertas para un mismo trabajo y elige la que mejor se adapte a ti.')}
          ${renderBenefitItem('Garantía de Satisfacción', '✨', 'Solo liberamos el pago cuando confirmas que el trabajo quedó perfecto.')}
        </div>
      </div>
    </section>
    ...
    <section class="py-24 2xl:py-32 bg-white overflow-hidden text-center">
      <div class="container">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 2xl:gap-24 relative">
          ...
          ${renderStepCircle(1, 'Publica tu proyecto', 'Describe lo que necesitas reparar o mejorar.')}
          ${renderStepCircle(2, 'Recibe presupuestos', 'Expertos interesados te enviarán sus ofertas.')}
          ${renderStepCircle(3, 'Revisa perfiles', 'Mira fotos de trabajos anteriores y calificaciones.')}
          ${renderStepCircle(4, 'Selecciona al mejor', 'Elige al profesional que más te dé confianza.')}
          ${renderStepCircle(5, 'Coordina detalles', 'Habla por el chat seguro y define fecha y hora.')}
          ${renderStepCircle(6, 'Paga con seguridad', 'El dinero se libera solo cuando el trabajo termina.')}
        </div>
      </div>
    </section>
      ...
      <div class="container flex flex-col lg:flex-row items-center gap-20 2xl:gap-32">
        <div class="lg:w-1/2 text-white text-center md:text-left">
           ...
           <div class="space-y-6 2xl:space-y-10">
              ${renderCheckFeature('Chat seguro con los profesionales', true)}
              ${renderCheckFeature('Seguimiento del estado de tu proyecto', true)}
              ${renderCheckFeature('Historial de pagos y facturas', true)}
           </div>
        </div>
    ...
    <section class="py-24 2xl:py-32 bg-background">
      <div class="container max-w-4xl 2xl:max-w-6xl">
        ...
        <div class="space-y-4 2xl:space-y-8">
          ${renderFaqItem('¿Cómo sé que el profesional es de confianza?', 'Cada profesional en Hommy pasa por un proceso de verificación de identidad, antecedentes y referencias. Además, puedes ver sus calificaciones reales de otros clientes.')}
          ${renderFaqItem('¿Tengo que pagar antes de que vengan?', 'Sí, el pago se realiza a través de la plataforma para activar el sistema Escrow. El dinero se mantiene seguro en Hommy y solo se le entrega al profesional cuando tú confirmas que el trabajo terminó.')}
          ${renderFaqItem('¿Qué incluye la garantía Hommy?', 'Nuestra garantía cubre problemas de calidad en la ejecución del servicio y mediación en caso de disputas entre el cliente y el profesional.')}
        </div>
      </div>
    </section>
...
function renderBenefitItem(title: string, icon: string, desc: string) {
  return `
    <div class="p-10 2xl:p-16 rounded-[40px] 2xl:rounded-[60px] bg-white border border-gray-100 shadow-sm text-center">
      <div class="text-5xl 2xl:text-7xl mb-8">${icon}</div>
...
function renderStepCircle(num: number, title: string, desc: string) {
  return `
    <div class="relative z-10 flex flex-col items-center text-center">
      ...
      <h4 class="text-xl 2xl:text-3xl font-bold text-secondary mb-4 px-4">${title}</h4>
      <p class="text-secondary-light text-sm 2xl:text-xl px-4 leading-relaxed">${desc}</p>
    </div>
  `;
}

function renderCheckFeature(text: string, isWhite: boolean) {
  return `
    <div class="flex items-center space-x-4 2xl:space-x-8">
      ...
      <span class="text-lg 2xl:text-2xl ${isWhite ? 'text-white' : 'text-secondary'}">${text}</span>
    </div>
  `;
}
  ...
  return `
    <div class="faq-item bg-white rounded-3xl border border-gray-100 overflow-hidden transition-all duration-300">
      <button class="faq-header w-full p-8 2xl:p-12 text-left flex items-center justify-between focus:outline-none">
        <span class="text-lg 2xl:text-3xl font-bold text-secondary">${question}</span>

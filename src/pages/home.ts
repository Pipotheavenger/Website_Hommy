    ...
    <section class="py-24 2xl:py-32 bg-white">
      ...
      <div class="container">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 2xl:gap-10">
          ${renderServiceCard('Fontanería', '🚿', 'bg-blue-50 text-blue-600')}
          ${renderServiceCard('Electricidad', '⚡', 'bg-yellow-50 text-yellow-600')}
          ${renderServiceCard('Pintura', '🎨', 'bg-primary/10 text-primary')}
          ${renderServiceCard('Jardinería', '🌿', 'bg-green-50 text-green-600')}
          ${renderServiceCard('Limpieza', '🧹', 'bg-pink-50 text-pink-600')}
          ${renderServiceCard('Mudanzas', '📦', 'bg-orange-50 text-orange-600')}
        </div>
      </div>
    </section>
    ...
    <section class="py-24 2xl:py-32 bg-background">
      ...
      <div class="container">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 2xl:gap-20">
          ${renderStepCard(1, 'Publica tu necesidad', 'Describe lo que necesitas y recibe presupuestos en minutos.')}
          ${renderStepCard(2, 'Compara y elige', 'Revisa perfiles, fotos y calificaciones de profesionales interesados.')}
          ${renderStepCard(3, 'Paga con seguridad', 'El pago se libera solo cuando confirmas que el trabajo terminó.')}
        </div>
      </div>
    </section>
        ...
        <div class="flex flex-col lg:flex-row items-center gap-16 2xl:gap-24">
          ...
          <div class="lg:w-1/2 space-y-8 2xl:space-y-12">
            ...
            <div class="space-y-6 2xl:space-y-10">
              ${renderBenefitItem('Profesionales 100% Verificados', 'Validamos antecedentes y experiencia de cada experto.')}
              ${renderBenefitItem('Garantía Hommy', 'Si algo no queda bien, te ayudamos a resolverlo.')}
              ${renderBenefitItem('Pagos Protegidos', 'Usamos sistema Escrow para proteger tu dinero.')}
              ${renderBenefitItem('Sin Costos Ocultos', 'Presupuestos transparentes antes de empezar.')}
            </div>
          </div>
        </div>
      </div>
...
function renderServiceCard(title: string, icon: string, colorClass: string) {
  return `
    <div class="group p-8 2xl:p-12 rounded-3xl 2xl:rounded-[48px] bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 text-center">
      <div class="w-16 h-16 2xl:w-24 2xl:h-24 ${colorClass} rounded-2xl 2xl:rounded-3xl flex items-center justify-center text-3xl 2xl:text-5xl mx-auto mb-6 group-hover:scale-110 transition-transform">
        ${icon}
      </div>
      <h3 class="font-bold text-secondary 2xl:text-2xl">${title}</h3>
    </div>
  `;
}

function renderStepCard(num: number, title: string, desc: string) {
  return `
    <div class="relative p-8 2xl:p-12 rounded-3xl 2xl:rounded-[48px] bg-white border border-gray-100 shadow-sm">
      ...
      <h3 class="text-xl 2xl:text-3xl font-bold text-secondary mb-4 mt-4 2xl:mt-8">${title}</h3>
      <p class="text-secondary-light 2xl:text-xl leading-relaxed">${desc}</p>
    </div>
  `;
}

function renderBenefitItem(title: string, desc: string) {
  return `
    <div class="flex items-start space-x-4 2xl:space-x-8">
      ...
      <div>
        <h4 class="font-bold text-secondary 2xl:text-2xl">${title}</h4>
        <p class="text-secondary-light text-sm 2xl:text-lg leading-relaxed">${desc}</p>
      </div>
    </div>
  `;
}

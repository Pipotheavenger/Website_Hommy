import"./main-BiKZ0cGa.js";function x(){const e=document.getElementById("app");e&&(e.innerHTML=`
    ...
    <section class="py-24 2xl:py-32 bg-white">
      <div class="container">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 2xl:gap-20">
          ${a("Validación de Identidad","🆔","Verificamos documentos de identidad oficiales mediante reconocimiento facial y tecnología biométrica.")}
          ${a("Antecedentes Judiciales","📜","Revisamos registros nacionales para asegurar que cada experto tenga un historial limpio y confiable.")}
          ${a("Verificación de Experiencia","⭐","Contactamos referencias personales y revisamos fotos de trabajos reales antes de otorgar el sello Hommy.")}
        </div>
      </div>
    </section>
        ...
        <div class="flex flex-col lg:flex-row items-center gap-20 2xl:gap-32">
          ...
          <div class="lg:w-1/2 space-y-12 2xl:space-y-20">
            ...
            <div class="space-y-8 2xl:space-y-12">
               ${s(1,"Contratas y depositas","El dinero se reserva de forma segura.")}
               ${s(2,"El profesional trabaja","Tú tienes la tranquilidad de que el pago está listo.")}
               ${s(3,"Confirmas y liberamos","Solo con tu aprobación final enviamos el pago.")}
            </div>
          </div>
        </div>
      </div>
  ...
  `)}function a(e,t,l){return`
    <div class="p-10 2xl:p-16 rounded-[40px] 2xl:rounded-[80px] bg-white border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-300 group text-center md:text-left">
      <div class="text-5xl 2xl:text-7xl mb-8 group-hover:scale-110 transition-transform origin-left">${t}</div>
      <h3 class="text-xl 2xl:text-3xl font-bold text-secondary mb-4">${e}</h3>
      <p class="text-secondary-light 2xl:text-xl leading-relaxed">${l}</p>
    </div>
  `}function s(e,t,l){return`

import"./main-Dbfd22MI.js";function d(){const t=document.getElementById("app");if(!t)return;t.innerHTML=`
    ...
    <section class="py-24 2xl:py-32 bg-white">
      <div class="container">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 2xl:gap-12">
          ${l("Primeros pasos","🚀","Aprende a crear tu cuenta y publicar tu primer proyecto.")}
          ${l("Pagos y Facturación","💳","Información sobre métodos de pago, facturas y el sistema Escrow.")}
          ${l("Seguridad y Privacidad","🛡️","Cómo protegemos tus datos y verificamos a los profesionales.")}
          ${l("Para Clientes","👤","Todo lo que necesitas saber como dueño de un hogar.")}
          ${l("Para Profesionales","🛠️","Consejos y reglas para destacar como experto Hommy.")}
          ${l("App Móvil","📱","Guías sobre el uso de nuestra aplicación iOS y Android.")}
        </div>
      </div>
    </section>
    ...
    <section class="py-24 2xl:py-32 bg-background">
      <div class="container max-w-4xl 2xl:max-w-6xl">
        ...
        <div class="space-y-4 2xl:space-y-8">
          ${o("¿Es gratis publicar un proyecto?","Sí, publicar un proyecto es totalmente gratuito. Solo pagas cuando contratas a un profesional y el pago se mantiene en garantía.")}
          ${o("¿Cómo verifican a los profesionales?","Realizamos una validación exhaustiva que incluye identidad, antecedentes judiciales y referencias de trabajos anteriores.")}
          ${o("¿Qué pasa si no estoy conforme con el trabajo?","Nuestro sistema de garantía interviene para mediar y asegurar que recibas el servicio por el que pagaste.")}
          ${o("¿Cómo funciona el pago en garantía?","Tu dinero se reserva en nuestra plataforma segura y solo se libera al profesional cuando confirmas la finalización exitosa.")}
        </div>
      </div>
    </section>
  ...
  `;const a=document.querySelectorAll(".faq-item");a.forEach(e=>{const s=e.querySelector(".faq-header");s==null||s.addEventListener("click",()=>{const i=e.querySelector(".faq-content"),n=e.querySelector(".faq-icon"),x=e.classList.contains("active");a.forEach(r=>{r!==e&&(r.classList.remove("active"),r.querySelector(".faq-content").style.maxHeight="0",r.querySelector(".faq-icon").style.transform="rotate(0deg)")}),x?(e.classList.remove("active"),i.style.maxHeight="0",n.style.transform="rotate(0deg)"):(e.classList.add("active"),i.style.maxHeight=i.scrollHeight+"px",n.style.transform="rotate(180deg)")})})}function l(t,a,e){return`
    <div class="p-10 2xl:p-16 rounded-[32px] 2xl:rounded-[60px] bg-white border border-gray-100 shadow-sm hover:shadow-2xl hover:border-primary/20 transition-all duration-300 group cursor-pointer text-center md:text-left">
      <div class="text-5xl 2xl:text-7xl mb-8 group-hover:scale-110 transition-transform origin-left">${a}</div>
  ...
  `}function o(t,a){return`
    <div class="faq-item bg-white rounded-2xl 2xl:rounded-[32px] border border-gray-100 overflow-hidden transition-all duration-300">
      <button class="faq-header w-full p-6 2xl:p-10 text-left flex items-center justify-between focus:outline-none">
        <span class="text-lg 2xl:text-2xl font-bold text-secondary">${t}</span>
